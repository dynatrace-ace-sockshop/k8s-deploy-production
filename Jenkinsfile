    ...
    stage('Update production versions with latest versions from staging') {
      steps {
        container('kubectl') {
          ...
          env.CARTS_DT_PROPS = "${sh(script:'kubectl -n staging get deployment  -o jsonpath=\'{.items[*].spec.template.spec.containers[0].env[?(@.name==\"DTCUSTOMPROP\")].value}\' --field-selector=metadata.name=carts', returnStdout: true)}"
          sh "printenv | sort"
